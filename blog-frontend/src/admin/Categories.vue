<template>
  <div class="manager">
    <div class="manager__header">
      <h2 class="manager__title">分类管理</h2>
      <button class="btn" @click="openAdd">+ 新建分类</button>
    </div>
    ...
    <div v-if="showForm" class="manager__overlay" @click.self="closeForm">
      <div class="manager__modal frosted-card">
        ...
        <form @submit.prevent="handleSubmit">
          ...
          <div class="editor__group">
            <label class="editor__label">排序</label>
            <input v-model.number="form.sort" type="number" class="editor__input" />
          </div>
...
function openEdit(item) {
  ...
  form.value = { name: item.name, slug: item.slug, sort: item.sort }
  showForm.value = true
}
...
async function handleSubmit() {
  try {
    if (editingId.value) {
      await updateCategory({ id: editingId.value, ...form.value })
    } else {
